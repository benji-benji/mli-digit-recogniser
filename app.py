...
conn = psycopg2.connect(
    dbname="digitdb", user="digituser", password="digitpass", host="db"
)
cur = conn.cursor()
cur.execute("""
...
st.sidebar.markdown(
    "This is an digit recogniser to classify handwritten digits. It uses a ResNet18 model trained on the MNIST dataset and can predict handwritten digits from 0 to 9. The model accuracy is 99.19% on MNIST testing data. "
)
...
def load_model():
    ...
    model.load_state_dict(
        torch.load("src/models/resnet18_mnist.pth", map_location=device)
    )
    model.eval()
    return model, device
...
if canvas_result.image_data is not None:
    ...
    label = st.number_input(
        "Enter the correct digit label (0-9):", min_value=0, max_value=9, step=1
    )
...
def fetch_logs(limit=10):
    conn = psycopg2.connect(
        dbname="digitdb", user="digituser", password="digitpass", host="db", port="5432"
    )
...
logs_df["timestamp"] = pd.to_datetime(logs_df["timestamp"]).dt.strftime(
    "%Y-%m-%d %H:%M:%S"
)
